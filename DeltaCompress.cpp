...
namespace Sorted_error
{
    struct Model
    {
        ...
        // So don't store the index value (0-719) isntead store each "largest"
        // index in a row. It would be uncommon if we need all 6.
        std::array<Periodic_update, 5> vec_largest =
        {{
             {6, 1}, {5, 1}, {4, 1}, {3, 1}, {2, 1}
        }};
    };
    ...
    auto sort(Sort_list to_sort) -> Sort_result
    {
        ...
        {
            auto largest_value = to_sort[j];
            auto largest_index = 0;

            for (unsigned i = 0; i < (size - j); ++i)
            {
                if (to_sort[i+j] > largest_value)
                {
                    largest_value = to_sort[i+j];
                    largest_index = i;
                }
            }

            result.indicies[j] = largest_index;
    ...
    // RAM: Reallly? unsort?
    auto unsort(const Sort_result& to_sort) -> Sort_list
    {
        static const auto size  = to_sort.indicies.size();
        auto result = to_sort.values;

        for (unsigned i = 0; i < size; ++i)
        {
            using std::swap;
            auto swap_index = ((size - i) - 1);

            swap
            (
                result[swap_index],
                result[swap_index + to_sort.indicies[swap_index]]
            );
        }
            ...
            for (unsigned i = 0; i < size; ++i)
            {
                ...
                if (has_error)
                {
                    ...
                    Sort_list to_sort =
                    {
                        ...
                        static_cast<unsigned>(vec_pos[1]),
                        static_cast<unsigned>(vec_pos[2]),

                        static_cast<unsigned>(vec_quat[0]),
                        static_cast<unsigned>(vec_quat[1]),
