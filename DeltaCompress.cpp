...
void Gaffer_tests()
{
...

    {
        ...
        auto quat = to_quat(gaffer);
        auto result = to_gaffer(quat);

        assert(result == gaffer);
    }
    {
        auto gaffer = Gaffer
        {
    ...
    for (int i = 0; i < 512; ++i)
    {
        for (int j = 0; j < 512; j += 11)
        {
            for (int k = 0; k < 512; k += 37)
            {
                ...
                auto largest = std::sqrt(1.0f - magnitude_squared(g));
        ...
        else
        {
            ...
            if (!quatSame)
            {
                // Test the new stuff
                {
                    ...
                    stats.rotor_bits.Update(MinBits(ZigZag(m.vec[0])));
                    stats.rotor_bits.Update(MinBits(ZigZag(m.vec[1])));
                    stats.rotor_bits.Update(MinBits(ZigZag(m.vec[2])));
                }
            }
